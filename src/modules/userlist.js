...
//**************generate userdata table to DOM*************//
const addUserDatatoDOM = (users) => {
    users.map((ele) => {
        let userTableBody = document.getElementById("table-body")
        let userRowData = document.createElement("tr");
        userRowData.setAttribute("id", ele.id)
        userRowData.innerHTML =
            `<td><input type="checkbox" id="checkbox${ele.id}" name="horns" class="rowCheckbox" onclick="selectedCheckBox(${ele.id})"></td>
            ...
            <img src="../icons/pen-to-square-regular.svg" alt="edit-button">
            </button>
            <button type="button" id="save${ele.id}" style="display: none" class="edit-button" onclick="saveEdit(${ele.id})"}>
            save
            </button>
            <button type="button" id="cancel${ele.id}" style="display: none" class="edit-button" onclick="cancelEdit(${ele.id})"}>
            cancel
            </button>
            <button type="button" id="deleteRow${ele.id}" class="icon-button delete" onclick="deleteRowItem(${ele.id})">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon-delete-svg" viewBox="0 0 448 512">
            <path
        ...
        userTableBody.append(userRowData)
    })
}
